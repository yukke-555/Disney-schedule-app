...
window.onload = () => {
    ...
    createTimeline();
    initMap();
    filterItems(); // これでattractionsが使える状態になる
    ...
    if (p) {
        // 少しだけ待ってから復元（念のため）
        setTimeout(() => applySharedData(p), 100);
    } else {
        loadMyPlan();
    }
};
...
function addSchedule(zone, item, shouldSave = true) {
    ...
    div.onclick = (e) => { e.stopPropagation(); div.remove(); saveMyPlan(); };
...
function filterItems() {
    ...
    // フィルタリング処理
    attractions.filter(i => {
        const matchSearch = i.name.toLowerCase().includes(searchTxt);
        const matchPark = (currentPark === 'ALL' || i.park === currentPark || (currentPark==='LAND' && i.park==='ランド') || (currentPark==='SEA' && i.park==='シー'));
        return matchSearch && matchPark;
    }).forEach(i => {
        const div = document.createElement('div');
        div.className = `item ${(i.park === 'ランド' || i.park === 'LAND') ? 'land' : 'sea'}`;
...
function saveMyPlan() {
    let data = [];
    document.querySelectorAll('.slot-dropzone').forEach((z, t) => {
        z.querySelectorAll('.schedule-item').forEach(i => data.push({t, n: i.innerText}));
    });
...
function setPark(p) { currentPark = p; filterItems(); }
function initMap() { map = L.map('map').setView([35.632, 139.881], 16); L.tileLayer('https://server.arcgisonline.com/ArcGIS/rest/services/World_Imagery/MapServer/tile/{z}/{y}/{x}').addTo(map); }
