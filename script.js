...
function setPark(park) {
    ...
    document.querySelectorAll('.filter-btn').forEach(btn => {
        ...
        if(btn.innerText.includes('すべて') && park === 'ALL') btn.classList.add('active');
        if(btn.innerText.includes('ランド') && park === 'LAND') btn.classList.add('active');
        if(btn.innerText.includes('シー') && park === 'SEA') btn.classList.add('active');
    });
    filterItems();
}
...
function filterItems() {
    ...
    const filtered = attractions.filter(item => {
        ...
        const matchPark = (currentPark === 'ALL' || item.park === currentPark);
    ...
    filtered.forEach(item => {
        ...
        div.className = `item ${item.park.toLowerCase()}`;
...
function createTimeline() {
    ...
    for (let h = 8; h <= 20; h++) {
        ["00", "30"].forEach(m => {
            ...
            slot.innerHTML = `<div class="time-label">${time}</div><div class="slot-dropzone"></div>`;
            slot.onclick = () => {
                if (selectedItem) {
                    const zone = slot.querySelector('.slot-dropzone');
                    const newItem = document.createElement('div');
                    newItem.className = `item schedule-item ${selectedItem.park.toLowerCase()}`;
                    newItem.innerText = selectedItem.name;
                    newItem.onclick = (e) => { e.stopPropagation(); newItem.remove(); };
