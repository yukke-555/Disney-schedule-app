...
function init() {
    ...
    if(p) {
        try {
            ...
            applyPlanData(data);
            alert("共有された予定を読み込みました！");
        } catch(e) { console.error("URLデータの復元に失敗"); }
    } else {
        loadMyPlan();
    }
}
...
function setPark(park) {
    ...
    document.getElementById('btn-' + park.toLowerCase()).classList.add('active');
    filterItems();
}
...
function filterItems() {
    ...
    attractions.filter(item => {
        ...
        const pClass = (item.park === 'ランド' || item.park === 'LAND') ? 'land' : 'sea';
...
function addSchedule(zone, item, save = true) {
    const div = document.createElement('div');
    const pClass = (item.park === 'ランド' || item.park === 'LAND') ? 'bg-land' : 'bg-sea';
    div.className = `schedule-item ${pClass}`;
    div.innerText = item.name;
    div.onclick = (e) => { e.stopPropagation(); div.remove(); removeMarker(item.name); saveMyPlan(); };
...
function saveMyPlan() {
    const data = [];
    document.querySelectorAll('.slot-dropzone').forEach((zone, t) => {
        zone.querySelectorAll('.schedule-item').forEach(i => data.push({t, n: i.innerText}));
    });
    localStorage.setItem('disney_plan', JSON.stringify(data));
}

function loadMyPlan() {
    const saved = localStorage.getItem('disney_plan');
    if(saved) applyPlanData(JSON.parse(saved));
}
...
function applyPlanData(data) {
    ...
    data.forEach(d => {
        ...
        if(item) addSchedule(zones[d.t], item, false);
    });
}

function createShareUrl() {
    const saved = localStorage.getItem('disney_plan');
    if(!saved || saved === "[]") return alert("予定が空です");
    const code = btoa(unescape(encodeURIComponent(saved)));
...
function copyUrl() { document.getElementById('share-url-display').select(); document.execCommand('copy'); alert('URLをコピーしました'); }
