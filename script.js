...
function init() {
    ...
    createTimeline();
    initMap();
    loadMyPlan(); // 自分の保存データを読み込む
}

// モーダル操作
function openGroupModal() {
    document.getElementById('group-modal').style.display = 'block';
    document.getElementById('group-initial-menu').style.display = 'block';
    document.getElementById('group-join-input').style.display = 'none';
    document.getElementById('group-result-display').style.display = 'none';
}
...
function createGroupCode() {
    ...
    slots.forEach((slot, index) => {
        ...
        items.forEach(item => {
            planData.push({ t: index, n: item.innerText });
        });
    });
    ...
    const code = btoa(unescape(encodeURIComponent(JSON.stringify(planData))));
...
function showJoinInput() {
    document.getElementById('group-initial-menu').style.display = 'none';
    document.getElementById('group-join-input').style.display = 'block';
}
...
function joinGroup() {
    ...
    try {
        const planData = JSON.parse(decodeURIComponent(escape(atob(code))));
        if (confirm("今の予定を上書きして、共有された予定を表示しますか？")) {
            applyPlanData(planData);
            closeGroupModal();
            saveMyPlan(); // 読み込んだ後に保存
        }
    } catch(e) {
        alert("有効なコードではありません。");
    }
...
function applyPlanData(data) {
    // 一旦クリア
    document.querySelectorAll('.slot-dropzone').forEach(z => z.innerHTML = "");
    markers.forEach(m => map.removeLayer(m.marker));
    markers = [];

    const slots = document.querySelectorAll('.slot-dropzone');
    data.forEach(d => {
        const itemObj = attractions.find(a => a.name === d.n);
        if(itemObj) addSchedule(slots[d.t], itemObj, false); // falseは再保存防止
    });
}

// 自動保存 (ブラウザを閉じても消えないように)
function saveMyPlan() {
    const slots = document.querySelectorAll('.slot-dropzone');
    let planData = [];
    slots.forEach((slot, index) => {
    ...
    localStorage.setItem('myDisneyPlan', JSON.stringify(planData));
}

function loadMyPlan() {
    const saved = localStorage.getItem('myDisneyPlan');
    if(saved) {
        applyPlanData(JSON.parse(saved));
    }
}
...
function addSchedule(zone, item, shouldSave = true) {
    ...
    if(shouldSave) saveMyPlan();
}

function copyGroupCode() {
    const copyText = document.getElementById('share-code-display');
    copyText.select();
    document.execCommand("copy");
    alert("コピーしました！LINE等で送ってください。");
}

/* --- その他の既存関数 (filterItems, initMap, switchTab 等) は維持してください --- */
...
function createGroup() {
    ...
    slots.forEach((slot, index) => {
        const items = slot.querySelectorAll('.schedule-item');
        items.forEach(item => {
            data.push({ timeIndex: index, name: item.innerText });
        });
    });
...
function joinGroup() {
    ...
    try {
        ...
        const slots = document.querySelectorAll('.slot-dropzone');
        data.forEach(d => {
            const itemObj = attractions.find(a => a.name === d.name);
            if(itemObj) addSchedule(slots[d.timeIndex], itemObj);
        });
        
        alert("グループの予定を読み込んだよ！");
        closeGroupModal();
    } catch(e) {
        alert("無効なコードです。");
    }
