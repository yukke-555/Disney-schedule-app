...
window.onload = () => {
    ...
    if (p) {
        try {
            // 日本語バグを防ぐデコード
            const decodedData = decodeURIComponent(escape(atob(p)));
            ...
            // エラーが出てもリストが表示されるようにloadMyPlanを呼ぶ
            loadMyPlan();
        }
    } else {
        loadMyPlan();
...
function saveMyPlan() {
    const data = [];
    document.querySelectorAll('.time-slot').forEach((slot, tIdx) => {
        slot.querySelectorAll('.schedule-item').forEach(item => {
            data.push({ t: tIdx, n: item.innerText });
        });
    });
...
function createShareUrl() {
    const saved = localStorage.getItem('disney_plan_v3');
    if (!saved || saved === "[]") return alert("予定が空っぽです");
...
function copyUrl() {
    const txt = document.getElementById('share-url-display');
    txt.select();
